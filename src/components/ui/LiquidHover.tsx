...
export function LiquidHover({
  ...
  useEffect(() => {
    ...
    const texType = extHalfFloat ? extHalfFloat.HALF_FLOAT_OES : gl.FLOAT;
...
`;
    ...
    function createFBO(w: number, h: number) {
      ...
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, Math.max(1, w), Math.max(1, h), 0, gl.RGBA, texType, null);
    ...
    function resizeCanvas() {
      ...
      res.w = Math.max(1, Math.round(baseResolution * ratio));
      res.h = Math.max(1, Math.round(baseResolution));
    }
  ...
  }, [imageSrc, videoSrc, resolution, cursorSize, intensity]);
